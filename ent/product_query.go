...
func (pq *ProductQuery) QueryCreatedBy() *UserQuery {
...
func (pq *ProductQuery) WithCreatedBy(opts ...func(*UserQuery)) *ProductQuery {
	query := &UserQuery{config: pq.config}
...
func (pq *ProductQuery) sqlAll(ctx context.Context) ([]*Product, error) {
	...
	if query := pq.withCreatedBy; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(product.CreatedByColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.product_created_by
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "product_created_by" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "product_created_by" returned %v for node %v`, *fk, n.ID)
			}
